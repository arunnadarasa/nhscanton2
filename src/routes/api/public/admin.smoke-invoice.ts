...
export const Route = createFileRoute("/api/public/admin/smoke-invoice")({
  server: {
    handlers: {
      POST: async () => {
        try {
          const { getCantonClient } = await import("@/lib/canton/client.server");
          const c = await getCantonClient();
          const trustCode = "GSTT";
          const icbCode = "LDN";
          const { partyTrust, partyIcb, partyAuditor } = await import("@/lib/nhs/canton.functions");
          const ts = Date.now();
          ...
          // Re-query as ICB to get the Invoice contract they can countersign
          const visible = await c.queryInvoices(partyIcb(icbCode));
          const found = visible.find((v) => v.contractId === created.contractId);
          if (!found) {
            return Response.json({ step: "query-invoice-as-icb", created, visibleCount: visible.length }, { status: 500 });
          }
